...
const logger  = require('portal-env').Logger('portal:apis','portal:users'); // Renamed variable
...
router.get('/:api', function (req, res, next) {
    ...
    async.parallel({
        ...
        async.parallel({
            ...
            let hasGroupSubscription = false;
            for (let i = 0; i < userInfo.applications.length; ++i) {
            ...
            if (!utils.acceptJson(req)) {
                res.render('apiis', {
                    ...
                    hasGroupSubscription: hasGroupSubscription,
